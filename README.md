...
- **"Used For" on medications:** Each medication records what it treats ("ADD/ADHD", "Blood pressure") as its own field rather than buried in free-text notes — shown as a sortable column on the Medications page, on the dashboard beneath the medication name, and, unlike notes, **on share links and the printed summary**, so a family member or doctor can see what a medication is for without asking
...
- **Calendar:** Unified timeline view of appointments, visits and calls, vaccinations, procedures, hospitalizations, and medications — month grid plus an agenda view with newest-first/oldest-first sorting, event-type filter pills, and a date-range filter
...
- **Audit log:** Every write, authentication event (login, logout, failed login, password change/reset), user-management action, and share link access logged with a specific action label, actor, timestamp, and detail; filterable in-app. Failed logins record **the address that was attempted** — including one matching no account — rather than an anonymous "unknown". AI queries store both the question and the assistant's answer (each truncated), so an exchange can be reviewed after the fact
