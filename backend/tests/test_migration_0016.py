...
def test_migration_0016_round_trip(fresh_db_url, monkeypatch):
    monkeypatch.setattr(app.config.settings, "database_url", fresh_db_url)
    cfg = Config("alembic.ini")

    command.upgrade(cfg, "head")
    eng = create_engine(fresh_db_url)
    cols = {c["name"] for c in inspect(eng).get_columns("visit_logs")}
    assert "visit_time" in cols
    eng.dispose()

    command.downgrade(cfg, "0015")
    eng = create_engine(fresh_db_url)
    cols = {c["name"] for c in inspect(eng).get_columns("visit_logs")}
    assert "visit_time" not in cols
    eng.dispose()
